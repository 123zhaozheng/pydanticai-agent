...
**Agent Dependencies (`pydantic_deep/deps.py`)**
...
- `get_current_active_user()`: Get active user (alias for clarity)
- `get_admin_user()`: Require admin privileges
- `check_permission()`: Check user's button/feature permissions via RBAC
- `get_dify_service()`: Dependency for external AI service integration
...
**Database Models (`models_example/`)**
Reference implementation for RBAC (Role-Based Access Control) system:
- `user.py`: User model with authentication, roles, and department relationships
  - JWT-based authentication with OAuth2
  - User activation status and admin flags
  - Many-to-many relationship with roles
- `role.py`: Role and permission models
  - `Role`: Core role definition with menu and button permissions
  - `RoleMenu`: Role-menu permission associations
  - `RoleButton`: Role-button permission associations
  - `AgentPermission`: Agent-specific permission control
- Supports multi-level permission control (menu access, button actions, agent usage)
